  ...
  <div class="min-h-screen bg-[#121212] text-white py-6">
    <div class="w-full px-2 md:px-4">
      ...
      <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 gap-3 px-2 md:px-6 mt-16 md:mt-0">
        ...
        <div v-if="premiumVideos.length === 0" class="col-span-full flex flex-col items-center justify-center py-16">
          <i class="fas fa-crown text-6xl text-gray-600 mb-4"></i>
          <h2 class="text-2xl font-bold text-gray-400 mb-2">No Premium Videos</h2>
          <p class="text-gray-500 text-center">Premium videos will be available soon.</p>
        </div>
        ...
        >
          ...
          <div class="relative w-full aspect-video bg-gray-800 rounded-lg overflow-hidden mb-2">
            ...
            <div class="absolute top-1 left-1 flex gap-1">
              ...
              <span class="bg-purple-600 text-white text-[10px] px-1.5 py-0.5 rounded">
                PREMIUM
              </span>
            </div>
...
export default {
  ...
  setup() {
    ...
    const fetchPremiumVideos = async (page = 1) => {
      try {
        loading.value = true
        // Fetch all videos and filter client-side to ensure we get paid videos
        const response = await axios.get(`https://ott.no1news.in/api/videos/?page=${page}&page_size=${pageSize * 2}&ordering=-created_at`)
        
        if (response.data) {
          // Debug: Log video types to see what's available
          console.log('All videos:', response.data.map(v => ({ id: v.id, title: v.title, video_type: v.video_type })))
          
          // Filter for paid videos only
          const paidVideos = response.data.filter(video => video.video_type === 'paid')
          
          console.log('Paid videos found:', paidVideos.length)
          
          const processedVideos = paidVideos.map(video => ({
            ...video,
          ...
          if (page === 1) {
            premiumVideos.value = processedVideos
          } else {
            premiumVideos.value = [...premiumVideos.value, ...processedVideos]
          }
    ...
    const loadMoreVideos = () => {
      if (hasMoreVideos.value) {
        fetchPremiumVideos(currentPage.value + 1)
      }
    }
